...
- `cave/PageFlipper*.sv` and `cave/RequestQueue*.sv` are hand-maintained
  framebuffer helpers with the original generated module interfaces preserved.
- `cave/CaveSyncReadMem.sv` is the shared implementation behind the small
  Chisel-generated memory wrappers such as `cacheEntryMem_*`, `channelStateMem_*`,
  and `ram_*`. The old module names remain as compatibility shells.
- `cave/CaveSyncQueue.sv` is the shared implementation behind the generated
  `Queue*` modules. The old queue module names remain as compatibility shells.
- `cave/CaveSinglePortRam.sv` and `cave/CaveTrueDualPortRam.sv` are shared
  wrappers around the existing Arcadia VHDL RAM blocks. The generated
  `SinglePortRam*` and `TrueDualPortRam*` module names remain as compatibility
  shells.
- `cave/CaveDualClockFIFO.sv` is the shared wrapper around the existing Arcadia
  VHDL dual-clock FIFO block. The generated `DualClockFIFO*` names remain as
  compatibility shells.
- `cave/Crossing.sv` is the hand-maintained tile-ROM clock-domain bridge used
  by the layer processors.
...
- `cave/CPU.sv` and `cave/CPU_1.sv` are hand-maintained wrappers around the
...
- `cave/CaveLayerProcessor.sv` is the shared hand-maintained tilemap renderer
  behind the three `LayerProcessor*` compatibility wrappers.
...
- `cave/SystemFrameBuffer.sv` is the hand-maintained HDMI-rotation framebuffer
  ...
  queue.
- `cave/BurstBuffer.sv` and `cave/BurstBuffer_1.sv` are hand-maintained
  burst-width adapters for the DDR/SDRAM download paths.
- `cave/BurstReadDMA*.sv` and `cave/BurstWriteDMA.sv` are hand-maintained
  burst DMA controllers with the original generated module interfaces preserved.
- `cave/BurstMemArbiter*.sv` are hand-maintained priority arbiters for the
  shared burst memory ports.
- `cave/AsyncMemArbiter.sv`, `cave/AsyncReadMemArbiter.sv`, and
  `cave/*DataFreezer*.sv` are hand-maintained async memory arbiters and
  response-stretching helpers for CPU/audio clock crossings.
- `cave/CaveReadCache.sv` is the shared implementation behind the generated
  `ReadCache*` compatibility wrappers used by program ROM, sound ROM, and
  layer tile ROM reads.
- `cave/Cache.sv` is the hand-maintained two-way write-back cache used by the
...
- `cave/ADPCM.sv` is the hand-maintained YMZ280B ADPCM nibble decoder used by
  the audio pipeline.
