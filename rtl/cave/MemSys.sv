...
module MemSys(
  ...
  wire        copyDmaStart = ~readyEnableReg & io_prog_done;
  wire        copyDmaBusy;
  wire        copyDmaInRd;
  ...
  wire [31:0] ddrSpriteTileRomAddr =
    io_spriteTileRom_addr + (io_gameConfig_sprite_romOffset + IOCTL_DOWNLOAD_BASE_ADDR);
  ...
  always @(posedge clock) begin
    ...
    if (reset)
      ...
      readyEnableReg <= readyEnableReg | (~copyDmaBusy & copyDmaBusyReg);
  end
