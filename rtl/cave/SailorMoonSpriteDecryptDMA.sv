...
module SailorMoonSpriteDecryptDMA(
  input         clock,
  input         reset,
  input         io_start,
  output        io_busy,
  ...
  localparam [23:0] RAW_ROM_BYTES = 24'h400000;
  ...
  function automatic [23:0] decrypt_byte_addr(input [21:0] dest_byte_addr);
    ...
    begin
      decryptIndex = {2'b00, dest_byte_addr} ^ 24'h0950c4;
      decrypt_byte_addr = {
        decryptIndex[23],
        decryptIndex[22],
        decryptIndex[21],
  ...
  wire [23:0] firstRawAddr = decrypt_byte_addr(22'd0);
  wire [23:0] nextByteRawAddr =
    decrypt_byte_addr({wordCounter, nextByteCounter});
  wire [23:0] nextWordRawAddr =
    decrypt_byte_addr({nextWordCounter, 3'b000});
  wire firstByteInRom = firstRawAddr < RAW_ROM_BYTES;
  wire nextByteInRom = nextByteRawAddr < RAW_ROM_BYTES;
  wire nextWordInRom = nextWordRawAddr < RAW_ROM_BYTES;
      ...
      case (stateReg)
        STATE_IDLE: begin
          if (io_start) begin
            ...
            byteCounter <= 3'd0;
            outWordReg <= 64'd0;
            readAddrReg <= io_src_base + {8'h00, firstRawAddr};
          end
        end
        ...
        STATE_READ_WAIT: begin
          if (io_mem_valid) begin
            ...
            if (wordDone)
              ...
              readAddrReg <= io_src_base + {8'h00, nextByteRawAddr};
            end
          end
        end
        ...
        STATE_WRITE: begin
          if (io_mem_wait_n) begin
            if (dmaDone)
              ...
              byteCounter <= 3'd0;
              outWordReg <= 64'd0;
              readAddrReg <= io_src_base + {8'h00, nextWordRawAddr};
            end
          end
        end
